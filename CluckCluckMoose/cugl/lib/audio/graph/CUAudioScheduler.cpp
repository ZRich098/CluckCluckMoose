 ...
 * @return true if initialization was successful
 */
bool AudioScheduler::init(Uint8 channels, Uint32 rate, Uint32 samples) {
    if (AudioNode::init(channels,rate)) {
        _buffer  = (float*)malloc(samples*channels*sizeof(float));
        return true;
    }
    return false;
}
...
void AudioScheduler::play(const std::shared_ptr<AudioNode>& node, Sint32 loop) {
    ...
    // This is the safe exchange order
    Uint32 size = _qsize.load(std::memory_order_relaxed)+1;
    _qskip.store(size,std::memory_order_release);

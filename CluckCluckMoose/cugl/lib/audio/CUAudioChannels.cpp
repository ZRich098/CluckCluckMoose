...
bool AudioChannels::init(Uint32 channels) {
    ...
    for(int ii = 0; ii <= _capacity; ii++) {
        ...
        slot = audio::AudioScheduler::alloc(_mixer->getChannels(),
                                            _mixer->getRate(),
                                            _mixer->getBufferSize());
        slot->setTag(ii);
    ...
    for(int ii = 0; ii <= 2*_capacity; ii++) {
        ...
        _pan1Pool.push_back(AudioPanner::alloc(_mixer->getChannels(),1,
                                               _mixer->getRate(),
                                               _mixer->getBufferSize()));
        _pan2Pool.push_back(AudioPanner::alloc(_mixer->getChannels(),2,
                                               _mixer->getRate(),
                                               _mixer->getBufferSize()));
    }
...
std::shared_ptr<AudioFader> AudioChannels::wrapInstance(const std::shared_ptr<Sound>& asset) {
    ...
    if (asset->getChannels() == 1) {
        if (_pan1Pool.empty()) {
            panner = AudioPanner::alloc(_mixer->getChannels(),1,
                                        _mixer->getRate(),
                                        _mixer->getBufferSize());
        } else {
        ...
        if (_pan2Pool.empty()) {
            panner = AudioPanner::alloc(_mixer->getChannels(),2,
                                        _mixer->getRate(),
                                        _mixer->getBufferSize());
        } else {

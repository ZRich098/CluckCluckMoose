...
#pragma mark Constructors
     ...
     * @return true if initialization was successful
     */
    bool init(Uint8 channels, Uint32 rate, Uint32 samples);
     ...
     * @return an allocated player with the given number of channels and sample rate
     */
    static std::shared_ptr<AudioScheduler> alloc(Uint8 channels, Uint32 rate, Uint32 samples) {
        std::shared_ptr<AudioScheduler> result = std::make_shared<AudioScheduler>();
        return (result->init(channels, rate, samples) ? result : nullptr);
    }

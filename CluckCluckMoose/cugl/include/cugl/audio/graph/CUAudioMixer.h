...
#pragma mark Constructors
     ...
     * @return true if initialization was successful
     */
    bool init(Uint8 width, Uint8 channels, Uint32 rate, Uint32 samples);
...
#pragma mark Static Constructors
     ...
     * @return a newly allocated mixer with the given number of channels and sample rate
     */
    static std::shared_ptr<AudioMixer> alloc(Uint8 width, Uint8 channels, Uint32 rate, Uint32 samples) {
        std::shared_ptr<AudioMixer> result = std::make_shared<AudioMixer>();
        return (result->init(width,channels, rate, samples) ? result : nullptr);
    }

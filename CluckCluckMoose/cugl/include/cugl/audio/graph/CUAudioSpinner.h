...
public:
     ...
     * @param field     The number of input channels
     * @param rate      The sample rate (frequency) in HZ
     * @param samples   The buffer size in samples
     *
     * @return true if initialization was successful
     */
    bool init(Uint8 channels, Uint8 field, Uint32 rate, Uint32 samples);
...
#pragma mark Static Constructors
     ...
     * @return a newly allocated spinner with the given number of input/output channels.
     */
    static std::shared_ptr<AudioSpinner> alloc(Uint8 channels, Uint8 field, Uint32 rate, Uint32 samples) {
        std::shared_ptr<AudioSpinner> result = std::make_shared<AudioSpinner>();
        return (result->init(channels,field,rate,samples) ? result : nullptr);
    }
...
#pragma mark Audio Graph
     ...
     */
    Uint32 getBufferSize()  const { return _capacity;  }

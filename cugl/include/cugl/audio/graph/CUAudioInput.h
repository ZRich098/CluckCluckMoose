 ...
 * time intervals. For example, on MacOS at 44100 Hz, a buffer size of 512 will
 * record twice (in immediate sucession) every 23 ms instead of once every
 * 11 ms (a buffer size of 1024 appers to record once ever 23 ms as expected).
 *
...
#pragma mark AudioManager Methods
    /**
     * Initializes the default input device with 2 channels at 44100 Hz.
     *
     * This device node will have a buffer capacity of
     * {@link AudioManager#getWriteSize()} samples, and an equal delay.  By
     * default, this value is 1024 samples. This means that, at 44100 Hz, the
     * recording delay is 23ms and the playback delay is an (additional)
     * 23 ms.  So 46 ms passes between data is captured at the hardware device
    ...
    /**
     * Initializes the given input device with 2 channels at 44100 Hz.
     *
     * This device node will have a buffer capacity of
     * {@link AudioManager#getWriteSize()} samples, and an equal delay.  By
     * default, this value is 1024 samples. This means that, at 44100 Hz, the
...
#pragma mark Data Access
     ...
     * on MacOS that the input device at a sample rate of 44100 Hz can only
     * record data every 23 milliseconds.  Hence so it can only (at best) keep

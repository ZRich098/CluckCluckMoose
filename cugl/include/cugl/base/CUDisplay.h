 ...
 * class.  That are set via static attributes (like {@link setFullscreen(bool)}
 * that are read by the method {@link start()}.  Setting these attributes
 * after the display has started will have no effect (though this may be a
 * a feature for later releases).
 *
...
#pragma mark Orientation
     ...
     * this orientation. This value is the display orientation at startup.
     *
     * This may or may not agree with the device orientation.  In particular,
     * it will not agree if the display orientation is locked (to say
     * portrait or landscape only).  However, this is the orientation that
     * is important for drawing to the screen.  To get the device orientation,
     * call {@link getDeviceOrientation()}.
     *
     ...
     * it will not agree if the display orientation is locked (to say
     * portrait or landscape only).  However, this is the orientation that
     * is important for drawing to the screen.
     *
     * @return the current display orientation.
     */
     ...
     *
     * The device orientation is a mobile device, as held by the user.  It
     * may or may not agree with the display orientation, which is how the
     * screen is drawn.  For example, if the screen is locked to landscape
     * orientation, it is still possible for the device to have portrait
     * orientation when it is held on its side.
     *

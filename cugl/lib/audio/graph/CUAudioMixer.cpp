...
std::shared_ptr<AudioNode> AudioMixer::attach(Uint8 slot, const std::shared_ptr<AudioNode>& input) {
    ...
    if (input->getChannels() != _channels) {
        CUAssertLog(false,"AudioNode has wrong number of channels: %d vs %d",input->getChannels(),_channels);
        return nullptr;
    } else if (input->getRate() != _sampling) {
        CUAssertLog(false,"AudioNode has wrong sample rate: %d vs %d",input->getRate(),_sampling);
        return nullptr;
    }

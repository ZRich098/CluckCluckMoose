...
/**
 * Initializes the default output device with 2 channels at 44100 Hz.
 *
 ...
 * By default, this is 512 samples. At 44100 Hz, this means that the node
 * has a potential lag of 12 ms, which is a single animation frame at 60
 ...
 * of 1024 for a sample rate of 44100 Hz corresponds to 23 milliseconds.
...
/**
 * Initializes the given output device with 2 channels at 44100 Hz.
 *
 * This device node will have a buffer (e.g. the number of samples that
 * the node plays at a time) of {@link AudioManager#getReadSize()} samples.
 * By default, this is 512 samples. At 44100 Hz, this means that the node
...
bool AudioOutput::init(const std::string& device, Uint8 channels, Uint32 rate, Uint32 buffer) {
    ...
    // TODO: This is not really good enough for real time resampling
    // Because the iPhone often has other ideas...
    SDL_BuildAudioCVT(&_audiocvt,want.format,want.channels,want.freq,_audiospec.format,_audiospec.channels,_audiospec.freq);
    if (_audiocvt.needed) {
        _audiocvt.len = std::ceil(buffer/_audiocvt.len_ratio) * channels * sizeof(float);
...
Uint32 AudioOutput::read(float* buffer, Uint32 frames) {
    ...
    if (input == nullptr || _paused.load(std::memory_order_relaxed)) {
        ...
        // At 512 frames, we can safely take 9ms to do this operation.
        Uint32 want = frames;
        float* temp = buffer;

        if (_audiocvt.needed) {
            float need = want/_audiocvt.len_ratio;
            temp = (float*)_audiocvt.buf;
            want = std::ceil(need);
            //std::memcpy(temp,_tester,want*_channels*sizeof(float));
            //temp += want*_channels;
            /*
            if (_frac > 0) {
                temp[0] = _prev0;
        ...
        Uint32 take = input->read(temp, want);
        if (take < want) {
            std::memset(temp+take*_channels*sizeof(float),0,(want-take)*_channels*sizeof(float));
        }
        
        // TODO: The SDL resampler is horrible and introduces discontinuities
        if (_audiocvt.needed) {
            Uint32 amt = frames*_channels*sizeof(float);
            SDL_ConvertAudio(&_audiocvt);
            CUAssertLog(_audiocvt.len_cvt >= amt, "Audio conversion has incorrect length: %d",_audiocvt.len_cvt);
            std::memcpy(buffer,_audiocvt.buf,amt);
        }
    }
    Timestamp end;
